...
app.config['MYSQL_DATABASE_HOST'] = '54.144.71.125'
...
@app.route('/resultados_fis', methods=["post"])
def resultados_fis():
    nombre = request.form['name']
    distrito = request.form['distrito']
    dia = request.form['dia']
...
@app.route('/saveDataFisioterapeuta', methods=['POST'])
def saveDataFisioterapeuta():
    nombre = request.form['name']
    distrito = request.form['distrito']
    especializacion = request.form['Especialización']
    certificaciones = request.form['Certificationes']
    ...
    for var in dias: 
        if (var != [] and hora_inicio != "" and hora_final != "" and (hora_inicio < hora_final) and nombre != "" and distrito != "" and especializacion != "" and certificaciones != ""):
            data = (nombre, distrito, var, hora_inicio, hora_final, especializacion, certificaciones)
            cursor.execute("insert into fisioterapeuta (nombre, distrito, dias, hora_inicio, hora_final, especializacion, certificaciones) values (%s, %s, %s, %s, %s, %s, %s);", data)
            conexion.commit()

    return redirect('/registro_fis')
